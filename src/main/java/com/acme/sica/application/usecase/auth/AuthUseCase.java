...
public class AuthUseCase {
    ...
    public AuthUseCase(UsuarioRepository usuarioRepository, JwtPort jwtPort, PasswordEncoderPort passwordEncoderPort,
    ...
    public LoginResponseDTO login(LoginRequestDTO request, String ipOrigen) {
        ...
        // REGLA DE NEGOCIO: Buscar usuario por username. Si no existe, registrar
        // auditoria y lanzar excepcion
        Usuario usuario = usuarioRepository.findByUsername(request.username().trim())
                .orElseThrow(() -> {
                    auditService.log(null, request.username(), "LOGIN_FAILED",
                            "Intento de inicio de sesion con usuario inexistente", ipOrigen);
                    return new SecurityException("Credenciales invalidas");
                });

        // REGLA DE NEGOCIO: Verificar si la cuenta esta previamente bloqueada por
        // intentos fallidos
        if (usuario.isBloqueado()) {
            auditService.log(usuario.getId(), usuario.getUsername(), "LOGIN_BLOCKED",
                    "Intento de inicio de sesion en cuenta bloqueada", ipOrigen);
            throw new SecurityException("La cuenta se encuentra bloqueada por exceder los intentos fallidos");
        }

        // REGLA DE NEGOCIO: Verificar hash de contraseña con BCrypt (!verifyPassword
        // indica contraseña incorrecta)
        if (!passwordEncoderPort.verifyPassword(request.password(), usuario.getPasswordHash())) {
            ...
            // REGLA DE NEGOCIO: Si acumula 3 o mas intentos y no es usuario admin, bloquear
            // cuenta
            boolean esAdmin = "admin".equalsIgnoreCase(usuario.getUsername());
            if (nuevosIntentos >= 3 && !esAdmin) {
                usuario.setBloqueado(true);
                usuarioRepository.update(usuario);
                auditService.log(usuario.getId(), usuario.getUsername(), "ACCOUNT_LOCKED",
                        "Cuenta bloqueada tras 3 intentos fallidos", ipOrigen);
                throw new SecurityException("La cuenta ha sido bloqueada por exceder los intentos fallidos");
            } else {
                usuarioRepository.update(usuario);
                auditService.log(usuario.getId(), usuario.getUsername(), "LOGIN_FAILED",
                        "Contrasenia incorrecta. Intento #" + nuevosIntentos, ipOrigen);
                throw new SecurityException("Credenciales invalidas");
            }
        }

        // REGLA DE NEGOCIO: Si la contraseña fue correcta, reiniciar contador de
        // intentos a 0
        usuario.setIntentosFallidos(0);
        usuarioRepository.update(usuario);

        // INTENCION: Generar token JWT firmado con 8 horas de vigencia y consultar
        // permisos por rol
    ...
    public void logout(AuthenticatedUserContext userContext, String tokenJti, String ipOrigen) {
        if (userContext != null && tokenJti != null) {
            ...
            auditService.log(userContext.userId(), userContext.username(), "LOGOUT",
                    "Cierre de sesion exitoso y token revocado", ipOrigen);
        }
    }
}
