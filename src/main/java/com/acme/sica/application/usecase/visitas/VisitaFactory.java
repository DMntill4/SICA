...
public class VisitaFactory {
    ...
    public static Visita createVisita(TipoVisita tipo, Long personaId, Long funcionarioId, String motivo, LocalDateTime fechaProgramada) {
        ...
        switch (tipo) {
            // REGLA DE NEGOCIO: Visita pre-registrada por funcionario inicia en estado APROBADO
            case PRE_REGISTRADA -> {
            ...
            // REGLA DE NEGOCIO: Visita Walk-In no anunciada inicia en PENDIENTE_APROBACION por el funcionario
            case NO_ANUNCIADA -> {
                visita.setEstadoVisita(EstadoVisita.PENDIENTE_APROBACION);
                visita.setFechaHoraProgramada(LocalDateTime.now());
            }
            // REGLA DE NEGOCIO: Pase temporal por carnet olvidado inicia en PENDIENTE_APROBACION_OLVIDO
            case PASE_TEMPORAL -> {
